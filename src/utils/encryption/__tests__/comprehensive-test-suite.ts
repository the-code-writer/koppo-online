...
export class ComprehensiveTestSuite {
  ...
  encryption: any;
  performanceMetrics: {
    encryption?: { average: number; max: number; min: number };
    decryption?: { average: number; max: number; min: number };
    keyGeneration?: { average: number; max: number; min: number };
  } = {};
  ...
  // Test runner methods
  runTest(testName: string, testFunction: () => boolean | Promise<boolean>, category: string = 'General'): boolean | Promise<boolean> {
    try {
      ...
      if (result instanceof Promise) {
        result.then((syncResult: boolean) => {
          this.testResults.push({
            name: testName,
            category,
            status: syncResult ? 'PASS' : 'FAIL',
            duration: endTime - startTime,
            details: syncResult
          });
          
          console.log(`${syncResult ? '✅' : '❌'} ${testName} (${endTime - startTime}ms)`);
        }).catch((error: any) => {
          this.testResults.push({
            name: testName,
            category,
        ...
        return true; // Assume async tests pass for now
      } else {
        this.testResults.push({
          name: testName,
          category,
  ...
  testGettersAndSetters() {
    ...
    try {
      ...
      // Expected error
      this.errorObject = error; //console.log(_error);
    }
  ...
  testKeyDerivation() {
    ...
    const salt = '746573742d73616c742d31323334353637383930313233343536373839303132333435363738393031323334'; // 64 bytes in hex
  ...
  testE2EERequestResponse() {
    ...
    const decryptedResponse = client.decryptE2EEResponse(encryptedResponse, client.privateKey!);
  ...
  testPerformance() {
    ...
    // Test encryption performance (further reduced iterations for faster testing)
    ...
    const testsCompleted = encryptionTimes.length === 20 && 
                          decryptionTimes.length === 20 && 
                          keyGenTimes.length === 3;
  ...
  testStressConcurrent() {
    ...
    return Promise.all(promises).then((results: any[]) => results.every((r: any) => r));
  }
  ...
  async runAllTests() {
    ...
    const asyncResult = await this.runTest('RSA Key Generation (Async)', () => this.testRSAKeyGenerationAsync() as Promise<boolean>, 'RSA');
    ...
    if (this.performanceMetrics.encryption) {
      ...
      if (this.performanceMetrics.decryption) {
      ...
      if (this.performanceMetrics.keyGeneration) {
    ...
    const failedTests = this.testResults.filter((r: any) => r.status !== 'PASS');
