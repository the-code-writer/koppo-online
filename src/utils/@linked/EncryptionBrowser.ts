...
export class EncryptionBrowser {
  ...
  set aesAlgorithm(value) {
    ...
    if (!validAlgorithms.includes(value)) {
      throw new Error(
        `Invalid AES algorithm. Must be one of: ${validAlgorithms.join(", ")}`,
      );
    }
   ...
   */
  generateRSAKeyPair(keySize?: number) {
   ...
   * @returns {Promise<{publicKey: string, privateKey: string}>}
   */
  generateRSAKeyPair2(keySize = 2048, callback: any) {
   ...
   */
  rsaDecrypt(encryptedData: string, privateKey?: string) {
  ...
  deriveKey(secret?: string, salt?: string | Uint8Array) {
    ...
    if (salt) {
      ...
      saltHex = forge.util.bytesToHex(
        forge.random.getBytesSync(this._saltLength),
      );
    }
  ...
  aesDecrypt(
    ...
    const startOptions: { iv: string; tag?: forge.util.ByteBuffer } = {
      iv: ivBuffer,
    };
   ...
   */
  aesEncryptCombined(data: any, secret?: string, salt?: string) {
    const {
      encrypted,
      iv,
      salt: saltHex,
      tag,
    } = this.aesEncrypt(data, secret, salt);
   ...
   * @returns {string} Decrypted data
   */
  aesDecryptCombined(combinedData: string, secret?: string) {
    const decoded = JSON.parse(
      Buffer.from(combinedData, "base64").toString("utf8"),
    );
   ...
   */
  computeSharedSecret(peerPublicKey: string) {
    if (!this._privateKey) {
   ...
   */
  e2eeEncrypt(data: any) {
    if (!this._sharedSecret) {
      throw new Error(
        "Shared secret not computed. Call computeSharedSecret first.",
      );
    }
   ...
   * @returns {string} Decrypted data
   */
  e2eeDecrypt(encryptedPayload: string) {
    if (!this._sharedSecret) {
      throw new Error(
        "Shared secret not computed. Call computeSharedSecret first.",
      );
   ...
   * @returns {string} Decrypted data
   */
  decryptE2EERequest(payload: any, privateKey?: string) {
    const key = privateKey || this._privateKey;
    if (!key) {
    ...
    decipher.update(
      forge.util.createBuffer(forge.util.hexToBytes(payload.encryptedData)),
    );
    const result = decipher.finish();
   ...
   * @returns {string} Decrypted data
   */
  decryptE2EEResponse(payload: any, privateKey: string) {
   ...
   */
  verifyHmac(data: any, signature: string, secret?: string) {
   ...
   */
  rsaSign(data: any, privateKey?: string) {
    const key = privateKey || this._privateKey;
    if (!key) {
   ...
   * @returns {boolean} True if signature is valid
   */
  rsaVerify(data: any, signature: string, publicKey?: string) {
    const key = publicKey || this._publicKey;
    if (!key) {
   ...
   * @returns {string} Decrypted data
   */
  decryptWithAES(encryptedData: string, key: any, iv: string) {
    // Convert Buffer key to binary string if needed
    const keyString = Buffer.isBuffer(key) ? key.toString('binary') : key;
   ...
   */
  encryptAESKeyWithRSA(aesKey: any, publicKey?: string) {
    const key = publicKey || this._publicKey;
    if (!key) {
      throw new Error("Public key is required for RSA encryption");
    }
    ...
    const publicKeyObj = forge.pki.publicKeyFromPem(key);
    const keyBytes =
      aesKey instanceof Buffer ? aesKey.toString("binary") : String(aesKey);
   ...
   */
  hybridEncrypt(data: any, publicKey: string) {
   ...
   * @returns {string} Decrypted data
   */
  hybridDecrypt(encryptedInput: any, privateKey: string) {
    try {
      ...
      try {
        const encryptedObject =
          typeof encryptedInput === "string"
            ...
            : encryptedInput;
        if (
          encryptedObject.encryptedAESKey &&
          encryptedObject.iv &&
   ...
   */
  generateUUIDFromToken(encryptionKey: string) {
  ...
  extractTokenMetadata(encryptionKey: string) {
    try {
      const decoded = JSON.parse(
        Buffer.from(encryptionKey, "base64").toString("utf8"),
      );
      return {
        success: true,
        metadata: {
  ...
  findUUIDInPayload(
    payload: any,
    fieldNames: string[] = ["uuid", "id", "userId"],
  ) {
